...
app.CalendarView = app.BaseView.extend({
    ...
    event_template: '<div data-uuid="<%=uuid%>" class="event">' +
                        '<a href="http://www.google.com/calendar/event?action=TEMPLATE&text=Video+Session+with+&dates=<%=month%> <%=day%> 2017 <%=start_time%>/<%=month%> <%=day%> 2017 <%=end_time%>=[description]&location=[location]&trp=false&sprop=&sprop=name:" class="title"><%=Event.day%></a>' +
                    '</div>',
