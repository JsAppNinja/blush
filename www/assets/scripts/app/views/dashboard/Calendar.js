...
app.CalendarView = app.BaseView.extend({
    ...
    event_template: '<div data-uuid="<%=uuid%>" class="event">' +
                        '<a href="http://addtocalendar.com/atc/google?utz=-300&uln=en-US&vjs=1.5&e[0][date_start]=<%=start_day%> <%= start_time %>&e[0][date_end]=<%=end_time%>&e[0][timezone]=<%=app.user.timezone%>&e[0][title]=Video%20Session%20with%20Kali&e[0][description]=Video%20Session%20with%20Kali&e[0][location]=Video%20Session&e[0][organizer]=Kali%20Rogers&e[0][organizer_email]=" class="title"><%=title%></a>' +
                    '</div>',
